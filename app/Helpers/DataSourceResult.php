...
// used for kendo ui grids to perform server side: sorting, filtering, pagination, grouping etc.
...
class DataSourceResult
{
    ...
    private $stringOperators = array('eq' => 'LIKE', 'neq' => 'NOT LIKE', 'doesnotcontain' => 'NOT LIKE', 'contains' => 'LIKE', 'startswith' => 'LIKE', 'endswith' => 'LIKE');

    private $operators = array('eq' => '=', 'gt' => '>', 'gte' => '>=', 'lt' => '<', 'lte' => '<=', 'neq' => '!=');

    private $aggregateFunctions = array('average' => 'AVG', 'min' => 'MIN', 'max' => 'MAX', 'count' => 'COUNT', 'sum' => 'SUM');
    ...
    private function total($tableName, $properties, $request, $override_count = null)
    {
        ...
        if (isset($override_count))
            $statement = $this->db->prepare($override_count);
    ...
    public function read($table, $properties, $request = null,$override_sql = null,$override_count = null)
    {
        $result = array();

        $propertyNames = $this->propertyNames($properties);

        $result['total'] = $this->total($table, $properties, $request, $override_count);

        $sql = sprintf('SELECT %s FROM %s', implode(', ', $propertyNames), $table);

        if (isset($override_sql))
            $sql = $override_sql;
